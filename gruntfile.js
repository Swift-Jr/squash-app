module.exports = function(grunt) {
  ...
  grunt.config(
    ...
    grunt.option("compress") ||
      process.env.GRUNT_COMPRESS ||
      grunt.config("env") === "live"
  );
  ...
  var serverList = ["php:server", "php:worker", "browserSync", "watch"];
  ...
  grunt.config("onPHPChange", ["copy:sourcephp", "copy:htaccess"]);

  grunt.config("onAssetChange", ["copy:assets", "copy:htaccess"]);
  ...
  grunt.config("browserSync", {
    ...
    options: {
      proxy:
        "<%= php.server.options.hostname %>:<%= php.server.options.port %>",
      watchTask: true,
      logLevel: "warn",
      reloadDelay: 3000,
      injectChanges: false,
  ...
  grunt.config("concat", {
    ...
    sourcejs: {
      src: ["<%=config.source %>js/*.js", "<%=config.source %>js/**/*.js"],
      dest: "temp/js/source.js"
    },

    appjs: {
      nonull: true,
      src: ["temp/js/vendor.js", "temp/js/source.js"],
      dest: "<%=config.build %><%=config.assets %>app.js"
    },

    appcss: {
      nonull: true,
      src: grunt.config("vendor_css").concat(["temp/css/less.css"]),
  ...
  grunt.config("copy", {
    ...
    phpunitci: {
      ...
      src: ["**", "!controllers/Welcome_test.php"],
  ...
  grunt.config("jshint", {
    ...
    all: ["gruntfile.js", "<%=config.source %>js/**/*.js"]
  });
  ...
  if (grunt.option("profiler-main") === "on") {
    grunt.log.writeln(
      "PROFILER: Server enabled - " +
        profiler_directives["xdebug.profiler_output_dir"]
    );
  ...
  grunt.config("shell", {
    codeignighter: {
      command:
        "git clone --branch master https://github.com/bcit-ci/CodeIgniter.git <%=config.ciroot %>"
    },
    ifx: {
    ...
    phpunit_install: {
      command:
        "curl https://phar.phpunit.de/phpunit-6.0.13.phar -o phpunit.phar; chmod +x phpunit.phar; sudo mv phpunit.phar /usr/local/bin/phpunit; phpunit --version;"
    },
    phpunit: {
      command:
        "git clone --branch master https://github.com/kenjis/ci-phpunit-test.git <%=config.phpunitroot %>"
    },
    phpcsfixer: {
      command:
        "curl -L https://cs.sensiolabs.org/download/php-cs-fixer-v2.phar -o php-cs-fixer; sudo chmod a+x php-cs-fixer; sudo mv php-cs-fixer /usr/local/bin/php-cs-fixer;"
    },
    tests: {
    ...
    tests_debug: {
      command:
        'cd build/application/tests/; export XDEBUG_CONFIG="remote_host=localhost remote_enable=1 remote_autostart=1"; phpunit ' +
        testPath +
        ";"
    },
    phpjwt: {
      command:
        "curl -sS https://getcomposer.org/installer | php; composer.phar require lcobucci/jwt"
    }
  });
  ...
  grunt.config("watch", {
    ...
    js: {
      files: ["<%=config.source %>js/*.js", "<%=config.source %>js/**/*.js"],
